...
Zotero_Preferences.Advanced = {
  ...
  submitDebugOutput: async function () {
    ...
    toggleDisabled(submitOutputButton, true); // We have to request permissions within a user gesture (even though we use this in Zotero.getSystemInfo())
  ...
  submitErrors: async function () {
    ...
    toggleDisabled(reportErrorsButton, true); // We have to request permissions within a user gesture (even though we use this in Zotero.getSystemInfo())
...
Zotero_Preferences.Components.ProxyPreferences = (_temp = class ProxyPreferences extends React.Component {
  ...
  updateState(name, value) {
    ...
    Zotero.Prefs.set('proxies.' + name, value); // Refresh prefs in the background page
...
Zotero_Preferences.Components.Proxies = class Proxies extends React.PureComponent {
  ...
  handleHostButtonClick(event) {
    ...
    this.setState(prevState => {
      var newState = {};
      var oldProxy = prevState.currentProxy;
      var newProxy = Object.assign({}, oldProxy); // Add host to end
      ...
      if (value == '+') {
        ...
        this.focusHostInput = true;
          ...
          newProxy.hosts = [...oldProxy.hosts.slice(0, prevState.currentHostIdx), ...oldProxy.hosts.slice(prevState.currentHostIdx + 1)]; // If this was the last host, select the previous one
  ...
  renderProxySettings() {
    ...
    var multiHost = currentProxy && currentProxy.toProperScheme.includes('%h') || currentProxy.toProperScheme.includes('%u'); // If a host exists in the last position and is empty, don't allow adding more
