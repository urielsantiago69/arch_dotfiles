...
(function () {
  ...
  Zotero.GoogleDocs.UI = {
    ...
    addKeyboardShortcuts: async function () {
      ...
      this.shortcut = Zotero.Utilities.Connector.kbEventToShortcutString(modifiers); // The toolbar button is managed by GDocs
      ...
      Zotero.Inject.addKeyboardShortcut(Object.assign(modifiers), Zotero.GoogleDocs.editField, textEventTarget); // Open Zotero menu shortcut, mimicking google doc's native shortcuts
    ...
    writeText: async function (text) {
      var evt; // On Safari this is the <body> element, but on other browsers it's a <div> in the <body>.
    ...
    insertLink: async function (text, url) {
      var selectedText = this.getSelectedText(); // If we do not remove the selected text content, then the insert link dialog does not
    ...
    getSelectedText: function () {
      var selection = document.querySelector('.docs-texteventtarget-iframe').contentDocument.body.textContent; // on macOS a U+200B ZERO WIDTH SPACE is the text content when nothing is selected here
    ...
    getSelectedFieldID: async function () {
      ...
      copyEventTarget.dispatchEvent(new CustomEvent('copy'));
      let selectionLink = this.getSelectedLink();
      let selectedText = this.getSelectedText(); // If there is already selected text do not modify the selection cursor
      ...
      if (!isZoteroLink(selectionLink) && !selectedText.length) {
        ...
        copyEventTarget.dispatchEvent(new CustomEvent('copy'));
        selectionLink = this.getSelectedLink();
        selectedText = this.getSelectedText(); // If the cursor was at the start of the text then no selection in the previous step occurred
  ...
  Zotero.GoogleDocs.UI.LinkInsertBubble = {
    ...
    async close(confirm = true) {
      ...
      let closePromise = this.waitToCloseAndMakeVisible(); // Make sure the dialog is fully opened before we attempt to close it

      if (confirm && urlInput.value) {
        Zotero.GoogleDocs.UI.setupWaitForSave();
        let applyButton = document.querySelector('.appsElementsLinkInsertionApplyButton'); // Likely a bug in the new google docs link insertion UI where pressing Enter
  ...
  Zotero.GoogleDocs.UI.Menu.Item = class extends React.Component {
    ...
    render() {
      ...
      let shortcutIdx = label.toLowerCase().indexOf(this.props.shortcutKey); // Underline shortcut key
